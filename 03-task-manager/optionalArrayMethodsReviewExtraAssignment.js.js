...
const lastNames = names.map((names) => {
  return names.trim().split(" ").pop();
});
...
const filteredNames = names.filter((names) => correctFormat.test(names.trim()));
...
const titleNames = names.map(toTitleCase);
console.log("titledNames", titleNames);
...
const results = names
  .filter((names) => correctFormat.test(names.trim()))
  .map(toTitleCase)
  .filter((name) => !name.split(" ").pop().toLowerCase().endsWith("z"))
  .map((name) => `Hi ${name}, sign up!`);
